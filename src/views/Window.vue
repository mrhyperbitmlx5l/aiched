<template>
	<div class="app focus" :style="{top:position.y+'px',left:position.x+'px',width:width+'px',height:height+30+'px'}">
		...
		<div v-for="(value,index) in resizeDirection" :key="index" v-show="true" :class="value" @mousedown="onResize($event)"></div>
	</div>
</template>
	...
	export default {
		...
		data() {
			return {
				...
				overlayShow: false,
				resizeDirection: ['resize-handle resize-handle-t', 'resize-handle resize-handle-b', 'resize-handle resize-handle-l', 'resize-handle resize-handle-r', 'resize-handle resize-handle-tl', 'resize-handle resize-handle-tr', 'resize-handle resize-handle-bl', 'resize-handle resize-handle-br']
			}
		},
		created() {
